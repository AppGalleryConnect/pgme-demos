...
export default class AudioMsgItem extends cc.Component {
  ...
  // 判断数字的正则表达式
  private number_reg = new RegExp('^[0-9]*$');
  ...
  private onPlayAudioMsg(filePath: string, code: number, msg: string) {
    LogUtil.printLog(
      '播放语音消息回调，code:' + code + ',msg:' + msg + ',filePath:' + filePath,
      LogType.AUDIO_MSG_TYPE,
      this.node,
    );
    const audioMsgItem = this.isFileId(filePath)
      ? GlobalData.audioMsgItems.find((audio) => audio.fileId.toString() === filePath)
  ...
  private onUploadAudioMsgFile(filePath: string, fileId: number, code: number, msg: string) {
    LogUtil.printLog(
      '上传语音消息，code:' + code + ',msg:' + msg + ',fileId:' + fileId,
      LogType.AUDIO_MSG_TYPE,
      this.node,
    );
    if (code === 0) {
      // 上传成功，组件文案，上传按钮消失,文件id 本地存储
      const audioMsgItem = GlobalData.audioMsgItems.find((audioMsgItem) => audioMsgItem.filePath === filePath);
      audioMsgItem.setUploadState(true);
      audioMsgItem.setFileId(fileId);
      // 通知事件
      this.node.dispatchEvent(new cc.Event.EventCustom(Constant.audioMsgEvent, true));
    }
  }
  ...
  private localPlayOrStopAudio() {
    // 本地播放和停止语音按钮事件
    const audioMsgItem = GlobalData.audioMsgItems.find((audioMsgItem) => audioMsgItem.filePath === this.filePath);
    audioMsgItem.isLocalPlay
      ? GlobalData.gameMediaEngine.playAudioMsg(this.filePath)
  ...
  private remotePlayOrStopAudio() {
    ...
    const audioMsgItem = GlobalData.audioMsgItems.find((audioMsgItem) => audioMsgItem.filePath === this.filePath);
    audioMsgItem.isRemotePlay
      ? GlobalData.gameMediaEngine.playAudioMsg(this.fileId.toString())
      : GlobalData.gameMediaEngine.stopPlayAudioMsg();
